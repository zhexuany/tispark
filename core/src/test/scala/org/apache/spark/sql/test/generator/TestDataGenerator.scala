...
object TestDataGenerator {
  ...
  def schemaGenerator(database: String,
                      ...
                      indices: List[Index],
                      colNames: List[String] = List.empty[String]): Schema = {
    ...
    val columnNames: List[String] =
      if (colNames.isEmpty) {
        dataTypesWithDescription.map { x =>
          val tp = x._1
          val colName = if (dataTypeMap(tp) == 1) {
          ...
          columnNameToDataTypeMap(colName) = x
          colName
        }
      } else {
        dataTypesWithDescription.zipWithIndex.foreach { x =>
          val idx = x._2
          val dataTypeDesc = x._1
          columnNameToDataTypeMap(colNames(idx)) = dataTypeDesc
        }
        colNames
      }
